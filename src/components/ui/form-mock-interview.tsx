...
export const FormMockInterview = ({ initialData }: FormMockInterviewProps) => {
  ...
  const cleanAiResponse = (responseText: string) => {
    ...
    if (jsonArrayMatch) {
      ...
      console.error("No JSON array found in response for parsing.");
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (initialData) {
        ...
        if (isValid) {
          const newDocRef = await addDoc(collection(db, "interviews"), {
            ...data,
            userId,
            questions: aiResult,
          ...
          toast(toastMessage.title, { description: toastMessage.description });
          navigate(`/generate/interview/${newDocRef.id}`, { replace: true });
          return;
        }
      }
      ...
      console.error(error);
      toast.error("Error..", {
        description: `Something went wrong. Please try again later. Check console for details.`,
      });
    } finally {
      setLoading(false);
    }
